...
4. **System Prompt** — the extension ships with a default system prompt (tuned for Krea 2 Turbo) that tells the LLM how to write the image prompt. It's also available as a **`Krea 2 Turbo (default)`** entry in the **System Prompt Presets** dropdown, so you can always switch back to it after editing. Edit the prompt directly in the settings textarea to suit your model. Save it under a name with **Save As**, switch between saved prompts via the presets dropdown, overwrite the selected preset with **Save**, and remove one with the 🗑 button. Presets are stored in your SillyTavern settings.
